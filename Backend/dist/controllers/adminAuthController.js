...
import { generateOtp, setOtp, verifyOtp, } from "../services/auth/adminOtpStore.js";
...
function getAdminEmails() {
    ...
    const senderEmails = extractEmails(cleanEnv(process.env.EMAIL_FROM || process.env.Email_From));
...
function signAdminToken(email) {
    ...
    return jwt.sign({ id: email, email, role: "admin" }, secret, {
...
export async function adminVerifyOtp(req, res) {
    const body = parseBody(adminVerifyOtpSchema, req.body);
    const email = body.email.toLowerCase().trim();
    const allowedAdmins = getAdminEmails();
    if (!allowedAdmins.includes(email)) {
        return sendError(res, "Unauthorized admin email", 401);
