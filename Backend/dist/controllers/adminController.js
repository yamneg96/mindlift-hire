...
import { adminFilterSchema, updateApplicationDecisionSchema, } from "../zod/application.js";
...
export async function listApplicationsForAdmin(req, res) {
    const query = parseQuery(adminFilterSchema, req.query);
...
export async function updateApplicationByAdmin(req, res) {
    if (!req.user) {
        return sendError(res, "Unauthorized", 401);
    }
    const body = parseBody(updateApplicationDecisionSchema, req.body);
    ...
    console.log(`[admin] decision ${body.status} for application ${application._id}`);
    return sendSuccess(res, application, "Application updated");
}
export async function deleteApplicationByAdmin(req, res) {
...
export async function getAdminStats(_req, res) {
    const [distinctApplicants, totalApplications, pendingApplications, approvedApplications, applicationsPerRole,] = await Promise.all([
