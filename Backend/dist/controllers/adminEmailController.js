...
export async function sendEmailToApplicants(req, res) {
    const body = parseBody(sendApplicantEmailSchema, req.body);
    const recipients = [
    ...
    await Promise.all(recipients.map((to) => sendApplicantNotificationEmail({
        to,
        subject,
    ...
    return sendSuccess(res, { sent: recipients.length, recipients }, "Email notifications sent");
}
