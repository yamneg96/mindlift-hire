...
export async function applyForRole(req, res) {
    if (!isRoleApplicationEnabled()) {
        return sendError(res, "Role applications are currently disabled. Please check again later.", 503);
    }
    ...
    for (const selectedRoleId of selectedRoleIds) {
        ...
        if (!selectedRole || selectedRole.status !== "open") {
            return sendError(res, "One or more selected roles are not available", 404);
        }
    }
    ...
    await sendApplicantNotificationEmail({
        ...
        roleTitle,
        message: "We have received your application and our team will contact you after the initial filtering process is completed.",
    }).catch(() => {
